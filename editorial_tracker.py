...
def list_items(connection: sqlite3.Connection, status: str | None) -> Iterable[sqlite3.Row]:
    if status:
        return connection.execute(
            """SELECT * FROM work_items WHERE status = ? ORDER BY due_date IS NULL, due_date""",
            (status,),
        )
    return connection.execute(
        """SELECT * FROM work_items ORDER BY due_date IS NULL, due_date"""
    )
...
def main() -> None:
    ...
    if args.command == "list":
        rows = list_items(connection, args.status)
        print_rows(rows)
        return
